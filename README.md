...
```
帧头(2B) + 命令(1B) + 长度(1B) + 数据(NB) + 校验(1B) + 帧尾(2B)
 AA 55      CMD       LEN       DATA...     XOR        0D 0A
```
...
| 帧头   | 2      | 固定 `0xAA 0x55`                      |
| 命令   | 1      | 命令字节 `0x00` ~ `0xFF`              |
| 长度   | 1      | LEN 后到校验前的字节数（即 DATA 长度） |
| 数据   | N      | 可变长度载荷                          |
| 校验   | 1      | 对命令+数据做 XOR 校验                |
| 帧尾   | 2      | 固定 `0x0D 0x0A`                      |

可配置项：CMD 位置（LEN 前/后）、校验算法（XOR / CRC16-MODBUS / CRC16-CCITT / CRC32 / 无）、Dummy 字节（帧头后插入的忽略字节）。
